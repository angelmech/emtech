...
namespace Audio
{
    ...
    public class AudioController : MonoBehaviour
    {
        ...
        [Header("Bridge Wind Audio")] public AudioSource[] bridgeWindAudios;

        [Header("Relaxing Music")] public AudioSource relaxingMusicSource;

        [Header("Player (XR Camera)")] public NetworkPrefabRef playerPrefab;
        private Transform playerCameraTransform;

        [Header("Height References (World Y)")] [Tooltip("Camera Y when player is on the ground")]
        public float groundLevelY = 0f;

        [Tooltip("Camera Y when player is fully on the bridge")]
        public float bridgeLevelY = 10f;

        [Header("Volume Settings")] public float groundAmbientMaxVolume = 0.5f;
        ...
        [Header("Smoothing")] public float volumeChangeSpeed = 2f;
        public float musicFadeSpeed = 1f;

        [Header("VR Input")] public InputActionProperty toggleMusicAction;
        ...
        void Update()
        {
            ...
            float targetGroundVolume = isRelaxingMusicMode
                ? 0f
                : Mathf.Lerp(
            ...
            float targetBridgeVolume = isRelaxingMusicMode
                ? 0f
                : Mathf.Lerp(
                    bridgeWindMinVolume,
                    bridgeWindMaxVolume,
